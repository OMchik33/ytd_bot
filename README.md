# ОБНОВЛЕНИЯ:
- **14.04.2025** - Переделал логику работы. // Объединил кнопки для скачивания видео, теперь бот сам определяет есть ли качество у видео, и предлагает выбрать. Если нет - скачать в максимальном качестве. // Возможность скачать обложку. // Исправил ошибки.
...
Добавляем туда код:
```bash
0/30 * * * * find /download -type f -mmin +30 -exec rm -f {} \;
```
Проверяем: `crontab -l`
