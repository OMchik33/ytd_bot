...
3) Установлен и настроен nginx (инструкция предполагает 443 порт, домен и SSL сертификаты. Если этого нет, то пробуйте по обычному http без сертификатов)
4) Установлены SSL сертификаты для домена, прикрепленного к серверу. Если сертификатов нет, попробуйте использовать через http, я данный вариант не испытывал.
